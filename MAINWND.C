      ...
      if(!DosCreateEventSem(NULL,
        ...
        for(;;) {
          ...
          if(fClockInTitlebars &&
            ...
            HWND hwndFrame = WinQueryActiveWindow(HWND_DESKTOP);

            if(hwndFrame &&
               WinWindowFromID(hwndFrame,
                               FID_TITLEBAR))
              PostMsg(WinWindowFromID(hwndFrame,
                                      FID_TITLEBAR),
                      UM_TIMER,
                      MPVOID,
                      MPVOID);
...
MRESULT EXPENTRY MainProc (HWND hwnd,ULONG msg,MPARAM mp1,MPARAM mp2) {
  ...
  static long  lastPage = 0,starty;
  static SHORT longestid = 0;
  ...
  switch(msg) {
      ...
      if(mp2 &&
        ...
        for(x = 0;x < c;x++) {
          if(WinSendMsg(hwndTasklist,
                        MM_QUERYITEM,
                        MPFROM2SHORT((SHORT)(x + IDM_TASK + 1),0),
                        MPFROMP(&mi)) &&
             mi.hItem)
            free((MENUPARMS *)mi.hItem);
        }
        _heapmin();
        for(x = 0;x < c;x++)
          WinSendMsg(hwndTasklist,
    ...
    case UM_SWITCHLIST:
      ...
      if(hwndTasklist) {
        ...
        char      *p;
        long       lx,ly,longest,this;
        MENUPARMS *mp;

        /* delete any old entries */
        WinSendMsg(hwnd,
                   UM_EMPTYMENU,
                   MPFROMSHORT(IDM_TASK),
                   MPFROMLONG(hwndTasklist));
        /* add new entries */
        memset(&mi,
               0,
               sizeof(mi));
        ...
        if(fMSEInTaskList) {
          ...
          if(mp) {
            ...
            mp->id = mi.id;
            mp->hwnd = hwndConfig;
            mp->hptr = (HPOINTER)WinSendMsg(mp->hwnd,
                                            WM_QUERYICON,
                                            MPVOID,
                                            MPVOID);
            if(!mp->hptr)
            ...
            this = 0;
            if(fDesktops) {
              if(WhichDesk(mp->hwnd,
                           NULL,
                           &lx,
                           &ly)) {
                this = 4;
                sprintf(mp->text + strlen(mp->text),
                        "\tDesk #%lu",
                        (lx + ((ly - 1) * 3)));
              }
            }
            if(this + strlen(mp->text) > longest) {
              longest = this + strlen(mp->text);
              longestid = mi.id;
            }
            WinSendMsg(hwndTasklist,
                       MM_INSERTITEM,
        ...
        if(ClipHwnd) {
          ...
          if(mp) {
            ...
            sprintf(mp->text,
                    ...
                    clipmgr);
            mp->id = mi.id;
            mp->hwnd = WinQueryWindow(ClipHwnd,QW_PARENT);
            mp->hptr = (HPOINTER)WinSendMsg(mp->hwnd,
                                            WM_QUERYICON,
                                            MPVOID,
                                            MPVOID);
        ...
        if(CalcHwnd) {
          ...
          if(mp) {
            ...
            strcpy(mp->text,
                   "MSE Calculator");
            mp->id = mi.id;
            mp->hwnd = WinQueryWindow(CalcHwnd,QW_PARENT);
            mp->hptr = (HPOINTER)WinSendMsg(mp->hwnd,
                                            WM_QUERYICON,
                                            MPVOID,
                                            MPVOID);
          ...
          for(c = 0;c < pswb->cswentry;c++) {
            if(pswb->aswentry[c].swctl.uchVisibility == SWL_VISIBLE &&
              ...
              if(mp) {
                ...
                mp->id = mi.id;
                mp->hswitch = pswb->aswentry[c].hswitch;
                mp->hwnd = pswb->aswentry[c].swctl.hwnd;
                mp->hptr = (HPOINTER)WinSendMsg(mp->hwnd,
                                                WM_QUERYICON,
                                                MPVOID,
                                                MPVOID);
    ...
    case WM_DRAWITEM:
      if(mp2) {
        ...
        if(own->hwnd == hwndTasklist &&
          ...
          rcl.yBottom += 1;
          rcl.yTop -= 1;
          if(own->fsAttribute & MIA_HILITED) {
            WinFillRect(own->hps,
                        &rcl,
                        CLR_DARKGRAY);
            GpiSetColor(own->hps,
          ...
          if((own->fsAttributeOld & MIA_HILITED) !=
             (own->fsAttribute & MIA_HILITED))
            own->fsAttributeOld = (own->fsAttribute &= ~MIA_HILITED);
          mp = (MENUPARMS *)own->hItem;
          WinDrawPointer(own->hps,
                         own->rclItem.xLeft + 8,
                         own->rclItem.yBottom + 2,
                         mp->hptr,
                         DP_MINI);
          p = strrchr(mp->text,'\t');
          if(p) {
          ...
          ptl.x = own->rclItem.xLeft + (yIcon / 2) + 10;
          ptl.y = own->rclItem.yBottom + starty;
          GpiMove(own->hps,
                  &ptl);
          GpiCharStringAt(own->hps,
                          &ptl,
                          strlen(mp->text),
                          mp->text);
          if(p &&
            ...
            GpiQueryTextBox(own->hps,
                            strlen(p),
                            p,
                            TXTBOX_COUNT,
                            aptl);
            ptl.x = (own->rclItem.xRight - own->rclItem.xLeft) -
                     (aptl[TXTBOX_TOPRIGHT].x + 2);
            ptl.y = own->rclItem.yBottom + starty;
            GpiMove(own->hps,
                    &ptl);
            GpiCharStringAt(own->hps,
    ...
    case WM_MEASUREITEM:
      if(mp2) {
        ...
        if(own->hwnd == hwndTasklist) {
          ...
          starty = 5;
          if(longestid == -1) {
            if(WinSendMsg(own->hwnd,
                          MM_QUERYITEMTEXT,
                          MPFROM2SHORT(longestid,sizeof(s)),
                          MPFROMP(s)))
              p = s;
          }
          else {
            ...
            if(WinSendMsg(own->hwnd,
                          MM_QUERYITEM,
                          MPFROMSHORT(longestid),
                          MPFROMP(&mi)) &&
               mi.hItem) {
              mp = (MENUPARMS *)mi.hItem;
              if(mp->text)
                p = mp->text;
            }
          }
          if(p &&
            ...
            GpiQueryTextBox(own->hps,
                            ...
                            TXTBOX_COUNT,
                            aptl);
            len = aptl[TXTBOX_TOPRIGHT].x + 8 + (yIcon / 2);
            if(strchr(p,'\t'))
              len += 16;
            height = max(height,(aptl[TXTBOX_TOPRIGHT].y -
                                 aptl[TXTBOX_BOTTOMLEFT].y) + 8);
            if(aptl[TXTBOX_BOTTOMLEFT].y < 1)
              starty = abs(aptl[TXTBOX_BOTTOMLEFT].y) + 3;
            else
              starty = 3;
          }
          len += 12;
          own->rclItem.xLeft = own->rclItem.yBottom = 0;
          own->rclItem.xRight = len;
          own->rclItem.yTop = height;
          return MRFROM2SHORT((SHORT)own->rclItem.yTop,
                              (SHORT)own->rclItem.xRight);
        }
      }
      break;
